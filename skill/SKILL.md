...
| 納品3点 | `hero.png`（**長辺2560**・256smp・デノイズ）＋ `loop.mp4`（**プロダクトフィルム**・長辺1080・24fps）＋ `model.glb`（動き込み・8MB以下） |
| 動画 | **プロダクトCMの型＝10〜12秒・3〜4カット**。最後のカットは hero と同じ構図で終わり1秒以上止まる。ループは最後→最初を**カットで**戻る（2026-09-23 Ryota決定・006〜）。カメラは必ず加減速。無音（BGMは持っていない）・文字なし |
...
4-b. **動画を組む（プロダクトフィルム）**：静止画が (a) で止まったら、雛形の `SHOTS` を題材に合わせて組み直す。
   > **なぜ**：第1期〜II 005 の動画は「ゆっくり回る1カットのループ」で、どの作品も同じ動きに見えた。
   > 2026-09-23 Ryota指示「全部プロダクトのいけてる動画みたいな感じにしてほしい」。
   - **3〜4カット・10〜12秒**。型（雛形のコメント）＝マクロ／スライド／回り込み／押し込み／光の走り／物の動き／決め。
     **毎回同じ並びにしない**（前作の `works.json` の `film.shots` を見て、並びを変える）
   - **寄り・中・引きを混ぜる**（全部同じ距離だと、カットを割っても同じ画に見える）。1つは必ずマクロ（100mm以上・浅いピント）
   - **最後は「決め」**＝hero の構図に動きながら入って止まる（`STILL_FRAME` は最後のフレーム＝hero はここから描かれる）
   - カメラは必ず加減速（`ease`／`ease_out`）。**カットは動いている途中で切る**。モーションブラーは使わない
   - 🔴 **舞台の端が写らないか**：カメラを動かすと、静止画では画面外だった床の端・壁の切れ目・光源が写る（雛形の仮置きでも2カット目に背景紙の端が写った）
   組んだら各カットの頭・中・終わりを描いて1枚に並べ、**Read して判定する**（hero だけ見ても動画は分からない）：
   ```bash
...
5. **本番**：🔴 **daily（launchd・`claude -p`）では、AI は動画（anim）を描かない。** 描くのは `daily.sh`。
   ...
   長辺1080で1フレーム描いて秒数を測り、`秒/フレーム × フレーム数（約260〜290）` が**120分**を超えないサンプル数にする（既定24・デノイズがあるので12〜16でも粒は出にくい）。
   参考：雛形の単純なシーンで 長辺1080・4smp＝約6秒/フレーム（大半はフレームごとの準備とデノイズ）。
   ```bash
...
```json
{
  ...
  "review_rounds": 4,
  "film": {"sec": 11.5, "shots": ["マクロ", "回り込み", "光の走り", "決め"]},
...
動画＝尺9.5〜13秒・カット3〜6・各カット1.5秒以上・止まったカットなし・最終フレームが hero と一致（006〜。〜005 は旧ループの基準）／glb 8MB以下・動きあり／film round 2周以上／
...
- [ ] 動画：`_contact.png` で、カットごとに見せているものが違うか・決めが hero と同じ構図か・マクロに見せ場があるか
