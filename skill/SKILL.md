...
description: >
  ...
  基準の作品を撮る→ルックを決める→シーン生成→基準と並べて3周以上の自己レビュー→本番レンダー→
...
| 自己レビュー | **最低3周**。毎周、基準と並べた1枚を Read し、`REVIEW.md` に記録する |
...
4. **テストレンダーと自己レビュー（最低3周・最重要）**：
   ...
   **`_sbs.png` を Read して判定する。自作だけを見ない。** 毎周 `REVIEW.md` に書く：
   ...
   直すこと：
   ```
   **3周目までに「基準と並べて見劣りしない」に届かなければ、ルックではなく題材か造形を替える**（パラメータで型は救えない＝#33）。
   上限は6周。4周目以降は `testhero`（長辺1600・128smp）で細部を見る。動きは `phases` の4枚で見る。
...
6. **点検と公開**：`works.json` に行を足してから
   ...
   作品フォルダに置くのは `hero.png` `loop.mp4` `model.glb` `script.py` `REVIEW.md` だけ（`_test*.png` `_sbs.png` は消す）。
...
動き量≥0.62・ループの閉じ≤2.2・静止率≤20%・尺5〜8秒／glb 8MB以下・動きあり／自己レビュー3周以上・基準の記載あり。
