...
| グレア | 新コンポジターのGlare **`Type='Streaks'`**・Threshold1.2・Strength0.35・Size0.55<br>🔴 **`'BLOOM'` は Blender 5.1 では TypeError。**それを `try/except: pass` が握りつぶし、**50作すべてが既定の Streaks で仕上がっていた**（#54）。**try で包まず明示的に書く。**<br>Bloom へ変えると51作目から絵が変わる＝**不変条件の変更＝Ryota判断**（比較実測は #54③） |
