...
> **色を1色も増やさずに質感の語彙だけ増やせる。** 実レンダー検証済みの5種（漆・陶・布厚・布薄・鉄）を以下に固定する。
>
...
## 5つのレシピ（Principled BSDF・Blender 5.1）
...
| **`nuno` 布（厚物）** | 0.80 | 0.20 | Sheen 0.55・Sheen Rough 0.25 | strength 0.004・noise_scale 0.05 | 24.8 ／ 59.1 |
| **`nuno_usu` 布（薄物）** | 0.66 | 0.28 | Sheen 0.55・Sheen Rough 0.25 | なし | 44.4 ／ 68.9 |
| **`tetsu` 鉄** | 0.50 | 0.32 | Metallic 0.35 | strength 0.012・noise_scale 0.09 | 22.7 ／ 63.3 |

健全域は 黒平均 14〜52 ／ 黒p98 56〜69（#45）。**5種とも合格。しかも5種とも base より p98 が高い**
＝質感を与えるほど黒は立体になる。

### 選び方（題材の実物の素材で選ぶ。迷ったら base）
- **漆**：塗り物・碗・箱・盆・楽器の胴・鞘。「深く沈んだ艶」がモチーフの本体であるもの
- **陶**：焼き物・石・土・瓦・臼・硯。**触ると少しざらつく**もの
- **布（厚物）**：縄・紐・帯・厚い幕。**塊として垂れる**もの
- **布（薄物）**：暖簾・幕・巻物・紙。**光を透かすほど薄い**もの。🔴 厚物の値を薄物に当てると黒が沈む（下記）
...
BLACK_RECIPES = {
    ...
    "nuno_usu": dict(rough=0.66, spec=0.28, sheen=0.55, sheen_rough=0.25),   # 薄物（暖簾・幕・紙）
...
## 🔴 薄物に厚物の値を当てない（2026-08-14・047 NOREN で実測）

球・立方・トーラスで採った `nuno`（Roughness 0.80／鏡面 0.20）を**実際の暖簾に当てたら黒が沈んだ**：

| 当てた値 | 黒平均 | 黒std | 黒p98 | 見え方 |
|---|---|---|---|---|
| 現行（0.58／0.22・Sheenなし） | 39.9 | 16.7 | 68.0 | 布に見えない（プラスチック） |
| 厚物 `nuno`（0.80／0.20） | 29.7 | 11.6 | 🔴 **53.1** | 沈んで襞が消える（#45の影絵側） |
| 薄物案1（0.58／0.22＋Sheen） | 40.2 | 16.6 | 68.0 | **現行とほぼ同じ＝Sheenが効いていない** |
| **薄物 `nuno_usu`（0.66／0.28＋Sheen）** | 44.4 | 16.0 | **68.9** | ✅ **襞が出て、黒も健全域** |

**わかったこと2つ**
1. **Sheen は粗い面でしか出ない。** Roughness 0.58 のままでは Sheen 0.55 を入れても絵が動かない（案1）。
2. **粗くすると黒が沈む。だから鏡面を同時に上げて相殺する**（0.66 に上げたら 0.20→0.28）。
   厚物は**塊なので沈んでも成立する**が、薄物は襞の陰影が命なので沈ませてはいけない。

**レシピの値は「その形で検証した値」でしかない。** 別の形に当てるときは必ず measure.py を通す
——3体のプリミティブで合格した値が、実際の造形で #45 を割ることがある。
